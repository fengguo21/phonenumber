    ...
    <div class='container'>
      ...
      <div class="phonecontainer">
        <div v-for='(item, index) in phoneList' :key='item.id' class='phoneitem' @click='selectPhone(item, index)'
          ...
          v-show='index < page * 100 - 1 && index > (page - 1) * 100 - 1'>
          {{ item.phone }}
        </div>
      </div>
      <div class='handle'>

        <el-button type="warning" @click='reset'>刷新重选</el-button>
        <el-button type="primary" @click='confirm'>确 定</el-button>
      </div>
      <div class='page'>
        <el-pagination :current-page='page' @current-change='pageChage' background layout="prev, pager, next"
          :page-size='100' :total="666">
        </el-pagination>
      </div>

    </div>

    <el-dialog title="办理号码" :visible.sync="dialogVisible" width="50%">
